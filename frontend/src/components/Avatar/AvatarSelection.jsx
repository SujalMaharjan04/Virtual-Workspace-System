    ...
    return(
        <div className = "bg-[#0F1117] h-screen flex flex-col items-center justify-center gap-8">
            ...
            <div className = "flex gap-6">
                {AVATARTS.map(avatar => (
                    <div key = {avatar.id} className = {`flex flex-col items-center gap-3 p-4 rounded-xl cursor-pointer border-2 transition-all duration-200 ${selected === avatar.is ? 'border-[#6C63FF] bg-[#6C63FF22]' : 'border-[#2E3153] bg-[#1A1D2E]'}`} onClick={handleSelect}>
                        {/*Avatar Preview */}
                        <div style = {{
                            width: '128px',
                            height: '128px',
            ...
            <button disable = {!selected} className = {`px-8 py-3 rounded-xl font-medium transition-color ${selected ? 'bg-[#6C63FF] text-white cursor-pointer hover:bg-[#5A52E0]' : 'bg-[#252840] text-[#4A5568] cursor-not-allowed' }`}>
                Enter Room
            </button>
        </div>
    )
